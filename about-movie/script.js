window.onload = function () {
  ...
  if (movieId === !null) {
    ...
    async function aboutMovieApiFetchingAlt () {
      const idUrl =  `https://imdb-movies-web-series-etc-search.p.rapidapi.com/${movieName}.json`;
    const idOptions = {
      method: 'GET',
      headers: {
      ...
      try {
        const idResponse = await fetch(idUrl, idOptions);
        const idResult = await idResponse.json();
        console.log(idResult)
        console.log(idResult.d[0])
        console.log(idResult.d[0].id)



        const url = `https://mdblist.p.rapidapi.com/?i=${idResult.d[0].id}`;
        const options = {
          method: 'GET',
          headers: {
        ...
        const urlForGenre = `https://imdb146.p.rapidapi.com/v1/title/?id=${idResult.d[0].id}`;
